...
async function isAuthorized(req: NextRequest): Promise<{ ok: boolean; adminId?: string }> {
  ...
  if (cronSecret && authHeader === `Bearer ${cronSecret}`) {
    return { ok: true, adminId: 'cron-buyer-purchase-guide-blast' };
  }
...
export async function POST(req: NextRequest) {
  ...
  const copy = buyerPurchaseGuideLifecycleCopy();
  ...
  if (recipients.length === 0) {
    return NextResponse.json({
      success: true,
      sent: 0,
      message: 'Todos los compradores alcanzables ya recibieron esta campaña.',
      alreadySent: alreadyNudged.size,
    });
  }
  ...
  try {
    if (adminId !== 'cron-buyer-purchase-guide-blast') {
  ...
  for (const user of recipients) {
    ...
    try {
      ...
      console.error('Buyer purchase guide blast failed for', user.email, e);
      failed++;
    }
  }
  ...
  if (adminId !== 'cron-buyer-purchase-guide-blast') {
    await logAuditEvent({
      ...
      targetId: campaign?.id ?? 'buyer-purchase-guide-manual-blast',
      details: {
        subject: copy.subject,
  ...
  return NextResponse.json({
    ...
    message: `Campaña compradores enviada: ${sent} correos/push (${failed} fallidos, ${skipped} omitidos)`,
  });
}
