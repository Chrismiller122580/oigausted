...
export async function GET(request: Request) {
  ...
  try {
    if (view === 'sellers') {
      ...
      earningsForSellers.forEach((e: any) => {
    ...
    const allReferrerIds = earnings.map((e: any) => e.referrerId)
    ...
    const referredCountMap = new Map(referredCounts.map((r: any) => [r.referredById, r._count.id]))
    ...
    const pageEarningsGroup = earnings.filter((e: any) => paginatedIds.includes(e.referrerId))
    const result = pageEarningsGroup.map((e: any) => {
      const user = users.find((u: any) => u.id === e.referrerId)
    ...
    result.sort((a: any, b: any) => b.pendingPayout - a.pendingPayout)
