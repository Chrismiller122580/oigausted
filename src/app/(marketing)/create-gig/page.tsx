...
export default function CreateGigPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
      <Link href="/" className="text-orange-600 hover:underline mb-6 inline-block">← Volver al inicio</Link>
      <h1 className="text-4xl font-bold mb-8">Crear Nuevo Gig</h1>

      <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 p-8 rounded-3xl border space-y-8">
        <div>
          <label className="block text-sm font-medium mb-2">Título del gig</label>
          <input type="text" name="title" value={formData.title} onChange={handleInputChange} required className="w-full px-4 py-3 border rounded-2xl focus:border-orange-500" placeholder="Ej: Asesoría Legal..." />
        </div>

        <div>
          <label className="block text-sm font-medium mb-2">Categoría</label>
          <select name="category" value={formData.category} onChange={handleInputChange} required className="w-full px-4 py-3 border rounded-2xl focus:border-orange-500">
        ...
        <div>
          ...
          <textarea name="description" value={formData.description} onChange={handleInputChange} required rows={6} className="w-full px-4 py-3 border rounded-3xl focus:border-orange-500" placeholder="Describe el servicio..." />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium mb-2">Precio (COP)</label>
            <input type="number" name="price" value={formData.price} onChange={handleInputChange} required className="w-full px-4 py-3 border rounded-2xl focus:border-orange-500" placeholder="150000" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-2">Tiempo de entrega (días)</label>
            <input type="number" name="deliveryTime" value={formData.deliveryTime} onChange={handleInputChange} required className="w-full px-4 py-3 border rounded-2xl focus:border-orange-500" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <label 
              ...
              className="cursor-pointer bg-orange-600 hover:bg-orange-700 text-white px-8 py-3 rounded-2xl font-medium transition flex items-center gap-2"
            >
            ...
            <span className="text-sm text-zinc-500">
              {images.length > 0 ? `${images.length} archivo(s) seleccionado(s)` : 'No se ha seleccionado ningún archivo'}
            </span>
          </div>
        </div>
