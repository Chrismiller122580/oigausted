      ...
      html = `
        <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
          <div style="text-align: center; margin-bottom: 32px;">
            ...
            <h1 style="color: #111; margin-top: 24px;">¡Bienvenido a OigaUsted!</h1>
          </div>
  ...
  try {
    ...
    if (result.error) {
      ...
      if (result.error.message?.toLowerCase().includes('not verified') || result.error.message?.toLowerCase().includes('domain')) {
        ...
        console.log('     RESEND_FROM_EMAIL="OigaUsted <onboarding@resend.dev>" npx tsx scripts/test-email.ts ' + toEmail + ' ' + emailType);
      }
      process.exit(1);
    }
